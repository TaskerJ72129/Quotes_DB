...
if (isset($_SESSION['admin'])) {
	...
	$tag_1_rs = get_rs($dbconnect, "SELECT * FROM `subject` 
		WHERE Subject_ID = $subject1_ID");
	$tag_1 = $tag_1_rs['Subject'];

	$tag_2_rs = get_rs($dbconnect, "SELECT * FROM `subject` 
		WHERE Subject_ID = $subject2_ID");
	if($tag_2_rs){
		$tag_2 = $tag_2_rs['Subject'];
	}

	$tag_3_rs = get_rs($dbconnect, "SELECT * FROM `subject` 
		WHERE Subject_ID = $subject3_ID");
	if($tag_3_rs){
		$tag_3 = $tag_2_rs['Subject'];
	}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	// edit database entry
	$editentry_sql = "UPDATE `quotes` SET `Author_ID` = '$author_ID', `Quote` = '$quote', `Notes` = '$notes',
...
<b>Quote Author:</b> &nbsp; 
	...
	<input class="add-field <?php echo $notes; ?>" type="text" name="notes"
	value="<?php echo $notes; ?>" placeholder="Notes (optional) ..."/>
